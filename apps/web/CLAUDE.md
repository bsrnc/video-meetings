...
- `app/meetings/[id]/page.tsx` — the meeting page, gated by `useSession()` the same way. Loads `GET /meetings/:id` with a bearer token and renders the title, the creation time, and the recording card (`components/recording-upload.tsx`). A 404 is reported as "This meeting does not exist, or it was deleted." rather than the API's own copy; 401 signs the user out. While the loaded meeting's `recordingStatus` is `UPLOADING`, a second effect polls `GET /meetings/:id` every `RECORDING_STATUS_POLL_INTERVAL_MS` (`lib/recording.ts`) until it isn't — this is what reflects an upload finishing (or failing) in another tab, or one still in flight from a previous visit, without a manual reload. `RecordingUpload` also gets `onUploadStarted` (sets `recordingStatus` to `UPLOADING` locally the moment this tab's own request goes out — the API sets that status at the same instant, so this is not a guess) and `onUploadFailed` (refetches once immediately, since a rejection may or may not have changed the server's status: a validation failure never touches it, a storage failure sets it to `ERROR`, and only the API knows which happened). The three fetches (initial load, poll, `onUploadFailed`) share one `fetchMeeting` callback.
...
`timeout` to clear `RECORDING_STATUS_POLL_INTERVAL_MS`.
