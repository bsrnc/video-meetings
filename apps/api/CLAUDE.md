...
- `src/auth/` — `AuthController` (`POST /auth/register` → 201, `POST /auth/login` → 200), `AuthService` (bcrypt password hashing, `@nestjs/jwt` signing), `dto/register.dto.ts` / `dto/login.dto.ts` (`class-validator`).
